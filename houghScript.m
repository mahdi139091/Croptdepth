...
for i = 1:numel(imglist)
    ...
    lines = houghlines(Im>threshold, 180*(thetaScale/pi), rhoScale, [rhos,thetas],'FillGap',5,'MinLength',10);
     ...
     %%ADDING SOME RESULT%%
   if i > 2 && i<=5
        filter1 = [0, -1, 0; -1, 5, -1; 0, -1, 0]; % SharpFilter (3*3)
        filter2 = 1/5*ones(5); % averaging
        [Filtered1] = myImageFilter(img, filter1);  
        [Filtered2] = myImageFilter(img, filter2);
        
        fname = sprintf('%s/%s_conv_sharp.png', resultsdir, imgname);
        imwrite(rescale(Filtered1,0,1), fname);
        
        fname = sprintf('%s/%s_conv_blur.png', resultsdir, imgname);
        imwrite(rescale(Filtered2,0,1), fname);  
